...
print("Loading raw jsonl...")

df = pd.read_json("codeforces_dataset.jsonl", lines=True)
...
for _, user in tqdm(df.iterrows(), total=len(df)):
    ...
    start = min(sub_times.min() if len(sub_times) else contest_times.min(),
                contest_times.min())
    ...
    while t + (FEATURE_DAYS + LABEL_DAYS) * DAY < end:
        ...
        attempts_per_ac = (
            np.mean(list(attempts.values())) if attempts else 0
        )

        # activity gaps
        days = sorted(set(s["creationTimeSeconds"] // DAY for s in window_subs))
        if len(days) >= 2:
            max_gap = max(np.diff(days))
        else:
            max_gap = FEATURE_DAYS
...
final_df.to_csv("cf_ml_dataset.csv", index=False)
